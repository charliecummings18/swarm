#ifndef CONSTANTS_H
#define CONTANTS_H

const double HEIGHT = 180.0;
const double WIDTH = 320.0;
const double DEPTH = 180.0;
const double MAX_SPEED = 100.0;
const double TIME_LIMIT = 60;
const double TIME_STEP = 0.01;
const int NUM_BOIDS = 200;
const double ALIGN_VISIBILITY = 30.0;
const double COHESION_VISIBILITY = 40.0;
const double SEPERATION_VISIBILITY = 30.0;
const double PREDATOR_VISIBILITY = 40.0;
const double ALIGN_FORCE = 0.1;
const double COHESION_FORCE = 0.4;
const double SEPERATION_FORCE = 0.4;
const double PREDATOR_FORCE = 0.5;
const double BUFFER_ZONE = 5;
const double TURN_FORCE = 5;
const int PREDATORS = 1;
const int OPTION = 0;
const int MASTER = 0;
const double VISIBILITY = 40.0;
const int DIMENSIONS = 3;


#endif
